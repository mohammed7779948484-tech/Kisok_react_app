...
| AC-03 | On a customer-kiosk device the Preparation experience is unreachable: even when a `preparation` account signs in successfully, the app shows the kiosk mismatch screen with a safe sign-out, and never mounts the `(preparation)` experience. This includes while the device-policy read is UNRESOLVED (first read not completed, failed, malformed, or `restrictions_pending`): the app holds at the startup target instead of mounting Preparation (remediation amendment 2026-09-04 — research R1/R2 proved the read can take seconds and no ordering guarantee exists; see review.md). Round 5 amendment 2026-09-04: UNRESOLVED also covers the native policy module being unexpectedly absent ON ANDROID (module absence is only expected on non-Android platforms — see review.md R5-01) and the window after a restrictions-change event invalidates a previous read until a new read resolves (see review.md R5-02). | Component/routing tests with a mocked device policy; derivation tests; runtime web preview with the policy source stubbed to kiosk; the cold-start ordering regression (fast auth + delayed native read proves Preparation never mounts). |
