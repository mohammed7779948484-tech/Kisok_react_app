...
| AC-09 | A dispatched release builds a signed APK, verifies its identity/signature/version/bundle, and refuses on mismatch   | workflow run; `verify-release-apk` unit tests incl. deliberate mismatch                                 |
| AC-10 | The verified APK is uploaded to the ManageEngine App Repository and KISOK is created or updated, matched by package | `publish-app` unit tests over the documented API contract; TENANT VALIDATION REQUIRED for the live call |
