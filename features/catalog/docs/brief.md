...
| AC-07 | Product Detail presents customer-safe product, media, generic variant/option and availability information without inventing Cart behaviour.                                                               | The requested product resolves or shows a safe not-found state; concrete variants are labelled by `title_override`, ordered option values, or a neutral ordered fallback when neither exists; option names/values are understandable, variant media falls back to product media and then the shared image fallback, availability is textual, and no quantity/Add-to-Cart/Checkout control is rendered. |
