...
| AC-11 | Boundary discipline: the files edited outside `features/catalog-cart-integration/**` are the two plan-justified edits (plus the catalog brief's AC-07 superseded-clause note applied at final-review closeout as FR-03 — the brief's own ID-stability rule; dispositioned in review.md) — `app/(customer)/_layout.tsx` (thin provider mount) and `features/catalog/screens/product-detail/product-detail-screen.tsx` (+ its test, the Add action as a Catalog-owning edit consuming the integration's public API). Cross-feature imports go through `@/features/catalog-cart-integration` and `@/features/cart` public surfaces only; no deep imports in either direction; no new shared primitive; no ESLint suppression | Public-API import tests; ESLint boundary rules green; diff review confined to the declared file set; generator conformity checks pass                                                          |
