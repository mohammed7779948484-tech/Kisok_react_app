...
| T05  | behavior | Acceptance AC-07, AC-08, AC-11                   | Integration convergence: public API pin, cross-surface flow, boundaries        | T03, T04   | RED: the convergence/key-equality pin suite is absent (C-T03-R1 note: the public index trio was wired in T03 — a structurally forced, documented deviation — so the RED driver is the absent pin suite, not an empty index; the merge/distinct/re-hydration cases re-verify T01–T04 behavior end-to-end) |
